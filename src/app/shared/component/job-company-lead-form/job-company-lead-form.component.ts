...
export class JobCompanyLeadFormComponent implements OnInit {
    ...
    this.addRecaptchaScript();
     this.getInTouchService.getJobCompanyContactSessions(this.jobId, null, null).subscribe((clmdata) => {
      if(clmdata && clmdata.sessionList && clmdata.sessionList.length>0){
        this.jobLeadsMeetingsdata = clmdata.sessionList;
      }
    }, err => {
  ...
  sheduleDateChanged(result: Date){
    ...
    const lsDate_ = new Date(this.sessionObject.date);
      if(this.jobLeadsMeetingsdata && this.jobLeadsMeetingsdata.length>0){
        for(var j=0; j<this.jobLeadsMeetingsdata.length; j++){
          let c = this.jobLeadsMeetingsdata[j];
  ...
  saveMeetingDataOnServer(meetingData) {
    ...
    this.getInTouchService.addSessionForJobCompanyLead(this.jobId, meetingData).then(data => {
      ...
      this.getInTouchService.getJobCompanyContactSessions(this.jobId, null, meetingData.lead_email).pipe(take(1)).subscribe((clmdata) => {
        if(clmdata && clmdata.sessionList && clmdata.sessionList.length>0){
          this.lmSession = clmdata.sessionList[0];
